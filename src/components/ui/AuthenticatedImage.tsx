...
/**
 * Resolves any image URL to an absolute URL:
 * - If it's already an absolute URL (Supabase, S3, data URL, blob), use it directly.
 * - If it's a relative path (e.g. /api/... or /uploads/...), prepend NEXT_PUBLIC_API_URL.
 */
...
export default function AuthenticatedImage({
  ...
  // Zoom / pan state for full modal preview
  ...
  if (error || !imgUrl) {
    return (
      <div className={cn('flex flex-col items-center justify-center bg-muted/20 text-muted-foreground p-3 text-center', className)}>
        <ImageIcon className="h-6 w-6 opacity-20 mb-1" />
        <span className="text-[10px] font-medium italic text-slate-400">Gagal memuat bukti</span>
      </div>
    );
  }

  const modalContent =
    isPreviewOpen && typeof window !== 'undefined'
      ? createPortal(
          <div
    ...
    <div className={cn('relative flex items-center justify-center overflow-hidden', className)}>
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-muted/10 z-10">
          <Loader2 className="h-5 w-5 animate-spin text-pln-cyan opacity-50" />
        </div>
      )}
      <img
        src={imgUrl}
